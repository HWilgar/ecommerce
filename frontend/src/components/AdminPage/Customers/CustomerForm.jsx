...
const CustomerForm = ({ customerId, setShowModal, toggleModal, setErrorMessage, setSuccessMessage, refetch }) => {
  const { data } = useFetchData(`https://ecommerce-kahpeh.vercel.app/api/v1/users/`);
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      await updateRole(`https://ecommerce-kahpeh.vercel.app/api/v1/users/${customerId}`, updatedRole, user.accessToken);
